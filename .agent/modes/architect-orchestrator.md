...
## STACK GUARDRAILS (MANDATORY)
- Next.js 16 App Router patterns only (no Vite/SPA routing assumptions).
- Server-first by default; push `'use client'` to interactive leaves.
- All `params/searchParams/cookies/headers` accesses must be awaited.
- Drizzle + Neon only (no Prisma).
- Biome only (no ESLint/Prettier workflows).
- Arcjet is the unified security layer.
