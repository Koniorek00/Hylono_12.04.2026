...
const RENTAL_PRODUCT_TECH_ID = 'hbot-st1700';
const RENTAL_PLANS = getRentalPlanOptions(RENTAL_PRODUCT_TECH_ID);
const DEFAULT_RENTAL_PLAN: RentalPlanOption | null =
    RENTAL_PLANS[1] ??
    RENTAL_PLANS[0] ??
    null;
...
const EnhancedRentalCheckout: React.FC<RentalCheckoutPageProps> = ({ onNavigate }) => {
    ...
    if (!selectedPlan) {
        return <LegacyRentalCheckoutFallback onNavigate={onNavigate} />;
    }
    ...
    const rentalItemsJson = useMemo(
        () =>
                    JSON.stringify([
                        {
                            techId: selectedPlan.productId,
                            quantity: 1,
                            monthlyPrice: selectedPlan.monthlyPrice,
                        },
                    ]),
        [selectedPlan.monthlyPrice, selectedPlan.productId]
    );

    const productTitle = selectedPlan.productTitle;
    const deposit = selectedPlan.deposit;
    const firstInvoiceEstimate = selectedPlan.monthlyPrice + deposit;
            ...
            <div className="min-h-screen bg-slate-50 pt-32 pb-24 px-6">
                <div className="max-w-2xl mx-auto bg-white border border-slate-100 rounded-3xl p-8 text-center">
                    ...
                    <p className="text-slate-600 mb-2">
                        We recorded your rental request for {productTitle} and will contact {deliveryEmail} with the
                        next steps, payment options, and delivery timing.
                    </p>
            ...
            <div className="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                >
                    ...
                    <div className="rounded-2xl border border-slate-200 p-4">
                        ...
                        <p className="text-sm font-semibold text-slate-900 mt-1">{formatEur(selectedPlan.monthlyPrice)}/mo</p>
                    </div>

                    <dl className="mt-5 space-y-2 text-sm">
                        <div className="flex justify-between">
                            <dt className="text-slate-500">Estimated first month</dt>
                        ...
                        <div className="flex justify-between">
                            ...
                            <dd className="text-slate-800">{formatEur(deposit)}</dd>
                        </div>
                        <div className="flex justify-between">
                ...
                >
                    <form action={rentalFormAction} className="space-y-8">
                        ...
                        <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600">
                            This form submits a rental request for manual review. We confirm payment
                            method, deposit timing, and delivery details with you after the request is
                            accepted.
                        </div>
