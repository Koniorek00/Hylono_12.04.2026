...
- Use Novu as the event-driven transactional layer and keep channel provider credentials outside application code. Feed it from site events and orchestration workflows.
...
- Read the self-host overview, API/webhook references, and current Hylono messaging flows before adding new templates or events.
...
### Never-assume list
- Never assume template variables, provider support, or webhook/event semantics without testing on the current platform version.
