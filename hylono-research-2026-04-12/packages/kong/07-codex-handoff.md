...
- Introduce Kong as the API boundary for internal/self-hosted services first, then selectively front public APIs where policy controls matter.
...
- Read the dependency map and current API surface before designing Kong routes.
...
### Never-assume list
- Never expose an admin plane publicly. Never add gateway complexity without a clear service-boundary plan.
