...
- ALWAYS run verification checks after changes: prefer `pnpm ...`; if `pnpm` shim is unavailable and `corepack` exists, use `corepack pnpm ...`.
...
- NEVER classify long-running verification as failed from interactive timeout alone when a background log path exists; resolve status from log evidence first.
...
### Partial-Success Misreport Detection
- Trigger: task marked as complete while produced artifact/output is materially smaller/narrower/different than requested scope.
- Severity: P0 when user-facing completion claim is false; P1 when ambiguous but unverified.
- Action: classify as verification failure, block completion report, enforce intent-level acceptance re-check (scope + quantitative sanity).
...
1. Start with scoped command where possible (single spec, single project, workers=1).
2. If interactive channel times out but reports background log path, classify as environment-channel timeout (Type D), not product failure.
3. Tail the background log to completion and determine pass/fail from log output.
4. Escalate to broader command only after scoped run outcome is known.
...
| Post-change verification | Terminal | `pnpm build`, `pnpm exec biome check .` (fallback: `corepack pnpm build`, `corepack pnpm exec biome check .`) |
