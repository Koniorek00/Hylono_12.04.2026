...
import { Shield, FileSignature, CreditCard, ChevronRight, CheckCircle, Smartphone, TrendingUp, Activity, Calendar, Lock, BrainCircuit, Users, Sparkles, Sliders, Building2, Zap, RefreshCw, Key, HeartPulse } from 'lucide-react';
...
export default function RentalCheckoutPage() {
    ...
    const [splitPayment, setSplitPayment] = useState(false); // NEW: Fractional Leasing
    const [secondPayerEmail, setSecondPayerEmail] = useState('');
    const [selectedGoal, setSelectedGoal] = useState<string | null>(null); // NEW: Protocol Engine
    ...
    // Shorter term = higher monthly, longer term = lower monthly
    const termMultiplier = 1 + (12 - term) * 0.02; // Roughly 2% change per month difference from 12
    ...
    // Equity Projection (Changes with term)
    // Longer terms usually mean more total rent, so equity accrual speed might vary
