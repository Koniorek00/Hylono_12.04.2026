...
export function getOperatorCredentialEntries(): OperatorCredentialEntry[] {
  ...
  return [
    ...
    },
    {
      ...
      endpoint: env.TWENTY_API_BASE_URL || "http://127.0.0.1:8107/rest",
      passwordLabel: "Bearer API key",
      passwordValue: env.TWENTY_API_KEY,
      commandLabel: "List local people",
      command: env.TWENTY_API_KEY
        ? `Invoke-RestMethod -Headers @{ Authorization = 'Bearer ${env.TWENTY_API_KEY}' } -Uri '${env.TWENTY_API_BASE_URL || "http://127.0.0.1:8107"}/rest/people?limit=10'`
        : undefined,
